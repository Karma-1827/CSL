...
- 自訂 Admin dashboard 顯示名冊/註冊/角色/配對/邀請統計。**2026-09-11 側邊欄新增「系統現況 / System status」小卡**(使用者要求,`accounts/services.py::count_online_users()`,`dashboard()` ADMIN 分支):顯示目前啟用中學期名稱(`Semester.objects.filter(is_active=True, starts_on__lte=today, ends_on__gte=today)`,若多個合作計畫剛好是同一個學期名稱只顯示一次,都沒有啟用中學期則顯示「無啟用中學期」)、目前在線人數、累計登入次數。「目前在線」是**未過期且已登入的 Django session 數**(逐一解碼 session payload 檢查 `_auth_user_id`),沿用既有 30 分鐘閒置逾時(`SESSION_COOKIE_AGE`)當作「在線」的定義,不是即時頁面瀏覽或 WebSocket 在線狀態(本專案刻意不做這兩種,見第 3 節系統邊界)。「累計登入次數」是 `AuditLog` 裡 `LOGIN_SUCCESS` 事件的總筆數(每次登入都算一次,不去重),沿用既有的 `CSLLoginView` 登入紀錄。
