...
- **2026-09-25(九十三)**:操作者 Claude Code(依使用者指示執行)。上一版 `ea08969` → 新版 `a84642e`(Admin「異常回報」紀錄表版面微調,詳見 `CLAUDE.md` 第 4.7 節/`docs/PROGRESS.md`)。使用者看到上一輪的版面後接著要求「紀錄時間 / Logged at的欄位縮小一點，可以分成兩行 日期/時間，通報者 / Reporter拉寬一點，然後細節不要只限於通報者 / Reporter的欄位，可以變成一整列」。純 template/CSS 改動(「細節」改成獨立一整列 `<tr class="incident-report-detail-row"><td colspan="7">`、通報者欄新增 `.col-reporter` 加寬、紀錄時間欄改成日期/時間分兩行),無 model/migration 變更。更新 1 項既有回歸測試涵蓋新版面斷言,437 項測試全數通過,`ruff`、`makemigrations --check --dry-run` 皆乾淨。無相依套件變更,有靜態資源變更(`collectstatic` 回報「1 static file copied ... 157 unmodified」,`app.css`?v=20260925-incident-report-table-layout)。部署前備份:`/var/backups/mpts/20260925-011232`(`db.dump` 786K、`media.tar.gz` 15.9M)。`git checkout --detach` 再次卡在根目錄 `CLAUDE.md`(同前幾次記錄的既有落差),已用 `sudo -n -u mpts git checkout HEAD -- CLAUDE.md` 補救。`makemigrations --check --dry-run` 確認無待套用 migration,`systemctl restart mpts-gunicorn.service`,`nginx -t` 通過(無 Nginx 設定變更,未 reload)。驗收:`curl` 確認新版 `app.css` 含 `.col-reporter`/`.incident-report-detail-row` 規則;用正式站真實 admin 帳號透過 Django test client(`force_login`,`secure=True`)確認 HISTORY 表格的「細節」已改為 `colspan="7"` 的獨立一列、「通報者」欄位套用加寬 class、「紀錄時間」正確顯示為日期/時間分兩行。`journalctl` 僅有既有的 gunicorn `Control server error` 無關訊息,`mpts-process-matching-state.timer` 為 active/waiting。臨時 sudo 授權沿用連線當下已開啟的狀態,依使用者先前指示「目前在做系統調整，可以先不關」維持開啟,本次未主動關閉。

- **2026-09-25(資料訂正，非程式碼部署)**:操作者 Claude Code(依使用者指示執行)。使用者要求把 3 位帳號(`41343115S`、`41300218E`、`41326014L`)從 NTNU 的 Tutee 身分轉為馬里蘭計畫的 Tutor 身分(其中一人稍早就是透過異常回報反映這個問題:修邱詩雯老師課、需要馬大實習,但註冊後身份是學生)。**部署前先唯讀盤點確認可以安全轉換**:三人皆完全沒有配對、課程、邀請、私訊、時數調整、配對排除紀錄,但各自有 1 筆自己送出的異常回報(`IncidentReport.reporter` 是 `on_delete=PROTECT`),代表**直接刪除帳號重新匯入這條路事實上行不通**(會被資料庫擋下,除非連他們自己的回報一起刪除),而且已完成註冊的帳號本來就無法透過公開流程重新走第二階段個人資料設定——因此改採「原地轉換」而非使用者原本提議的「刪除重新匯入」。已先在本機用同結構的假資料(`DRYRUN-T1/T2/T3`)完整跑過一次腳本並驗證後才對正式站執行,結束後已從本機清除。轉換內容:`RosterEntry.role`→`TUTOR`、`program`→`MARYLAND`、`education_level`→`BACHELOR`(已向使用者確認三人皆為大學部,馬里蘭 Tutor 資格規則要求必須是大學部,見 `CLAUDE.md` 4.3 節);`User.role`→`TUTOR`;刪除舊的 `TuteeProfile`,建立新的 `TutorProfile`(性別/母語/國籍/系所沿用舊資料,教學能力自評/教學簡介/可配合時段刻意留白,需要當事人自行登入 `/profile/` 填寫,不可用 Tutee 自己的華語程度自評頂替);寫入 `ADMIN_ROLE_CONVERTED` `AuditLog`(記錄轉換前後的角色/計畫/學制);三人各自的異常回報標記為已紀錄,備註說明帳號身分已調整。部署前備份:`/var/backups/mpts/20260925-005434`(`db.dump` 785K、`media.tar.gz` 15.9M)。轉換腳本透過 `scp` 傳到 VM 暫存目錄、以 `sudo -n -u mpts ... python manage.py shell < 腳本` 執行(避免多層 shell 轉義中文字串的既有已知風險),執行後腳本已從 VM 刪除。驗收:查詢正式站確認三人的 `User.role`/`RosterEntry.role`/`program`/`education_level` 皆已正確更新、`TutorProfile` 已建立且 `TuteeProfile` 已移除、異常回報狀態為已紀錄;並用各自帳號透過 Django test client(`force_login`,`secure=True`)確認 `/dashboard/` 回應 200 且正確顯示 Tutor 專屬的口語能力證明上傳區塊。無程式碼變更,無 migration,不涉及 `git checkout`/`collectstatic`/`systemctl restart`。臨時 sudo 授權沿用連線當下已開啟的狀態,依使用者先前指示「目前在做系統調整，可以先不關」維持開啟,本次未主動關閉。三人接下來仍需自行：①登入 `/profile/` 補填教學能力自評、教學簡介、可配合時段;②上傳口語能力證明並等待 Admin 審核通過,才能開始被馬里蘭 Tutee 配對。**同日使用者接著要求把三筆異常回報的「紀錄備註」文字改成更明確的操作指示**:「系統身分已更新為老師，請至「個人資料」補填教學能力自評、教學簡介、可配合時段，以及上傳修課證明。謝謝」(其中「修課證明」呼應馬里蘭計畫的資格認定方式是修課名單、不是口語考試,見 `CLAUDE.md` 第 2 節 2026-09-24 條目)。已再次備份(`/var/backups/mpts/20260925-010233`)後,用同一套 scp+shell 慣例只更新這三筆 `IncidentReport.resolution_note` 欄位,其餘欄位(`status`/`resolved_by`/`resolved_at`)不變,執行後已核對新文字正確寫入。

- **2026-09-25(九十二)**:操作者 Claude Code(依使用者指示執行)。上一版 `a9ad8cb` → 新版 `ea08969`(Admin「異常回報」通報者補上學號、紀錄表新增可展開細節,詳見 `CLAUDE.md` 第 4.7 節/`docs/PROGRESS.md`)。使用者要求「通報者 / Reporter 名字後方顯示學號，比較方便查找」與「紀錄的部分，能不能在通報者 / Reporter下方新增個『細節』，點開會呈現他的通報內容」。純 template/CSS 改動(`templates/dashboard/index.html`、`static/css/app.css` 新增 `.incident-report-detail`),無 model/migration 變更。更新 1 項既有回歸測試,437 項測試全數通過,`ruff`、`makemigrations --check --dry-run` 皆乾淨。無相依套件變更,有靜態資源變更(`collectstatic` 回報「1 static file copied ... 157 unmodified」,`app.css`?v=20260925-incident-report-details)。部署前備份:`/var/backups/mpts/20260925-004206`(`db.dump` 783K、`media.tar.gz` 15.9M)。`git checkout --detach` 再次卡在根目錄 `CLAUDE.md`(同前幾次記錄的既有落差),已用 `sudo -n -u mpts git checkout HEAD -- CLAUDE.md` 補救。`makemigrations --check --dry-run`/`migrate --plan` 皆確認無待套用 migration,`systemctl restart mpts-gunicorn.service`,`nginx -t` 通過(無 Nginx 設定變更,未 reload)。驗收:`curl` 確認新版 `app.css` 含 `.incident-report-detail` 規則;用正式站真實 admin 帳號透過 Django test client(`force_login`,`secure=True`)確認 PENDING 卡片與 HISTORY 表格的「通報者 / Reporter」皆正確顯示學號,HISTORY 表格的「細節 / Details」收合區塊點開後正確顯示對應的真實通報內容全文,課堂通報(`ClassAlert`)的「通報者」文字未受影響(本次刻意不動,僅限異常回報)。`journalctl` 僅有既有的 gunicorn `Control server error` 無關訊息,`mpts-process-matching-state.timer` 為 active/waiting。臨時 sudo 授權沿用連線當下已開啟的狀態,依使用者先前指示「目前在做系統調整，可以先不關」維持開啟,本次未主動關閉。

- **2026-09-25(九十一)**:操作者 Claude Code(依使用者指示執行)。上一版 `b564f87` → 新版 `a9ad8cb`(公告內容新增英文版本,中英文皆為必填,詳見 `CLAUDE.md` 第 4.11 節/`docs/PROGRESS.md`)。使用者確認前一次卡片式改版沒問題後,接著要求「內容加上英文」。新增 `Announcement.content_en` 欄位,`AnnouncementForm` 要求中英文皆為必填,顯示比照全站既有的「長雙語句用換行」慣例(`.bilingual-note`)在同一段落用 `<br>` 分隔。新增 2 項回歸測試,437 項測試全數通過,`ruff`、`makemigrations --check --dry-run` 皆乾淨。**含 1 個 migration**:`accounts.0023_announcement_content_en_alter_announcement_content`(新增可為空的 `content_en` 欄位+`content` 欄位僅 `verbose_name` 標籤變更,無資料異動)。無相依套件變更、無靜態資源異動(這次只改 Python/template,`collectstatic` 回報「0 static files copied」)。部署前備份:`/var/backups/mpts/20260925-002937`(`db.dump` 783K、`media.tar.gz` 15.9M)。`git checkout --detach` 再次卡在根目錄 `CLAUDE.md`(同前幾次記錄的既有落差),已用 `sudo -n -u mpts git checkout HEAD -- CLAUDE.md` 補救。`migrate` 套用 1 個 migration 後 `systemctl restart mpts-gunicorn.service`,`nginx -t` 通過(無 Nginx 設定變更,未 reload)。**資料建立(非程式碼變更)**:部署完成後,把本機準備好的一次性腳本(逐一以 `content` 中文全文比對每則 `Announcement`,`.update(content_en=...)` 寫入對應英文翻譯)透過 `scp` 傳到 VM 暫存目錄,再用 `sudo -n -u mpts ... python manage.py shell < 腳本` 執行(避免多層 shell 轉義中文字串的既有已知風險),為既有 5 則公告逐一補上英文翻譯,執行後腳本已從 VM 刪除。驗收:用正式站真實 Tutee 帳號透過 Django test client(`force_login`,`secure=True`)確認公告卡片同時顯示中英文內容(`<br>` 分隔於同一段落),5 則公告 `content_en` 皆非空。`journalctl` 僅有既有的 gunicorn `Control server error` 無關訊息,部署期間有真實使用者正常完成課程確認、送出邀請、瀏覽 Dashboard,`mpts-process-matching-state.timer` 為 active/waiting。臨時 sudo 授權沿用連線當下已開啟的狀態,依使用者先前指示「目前在做系統調整，可以先不關」維持開啟,本次未主動關閉。

- **2026-09-25(九十)**:操作者 Claude Code(依使用者指示執行)。上一版 `8ec2dab` → 新版 `b564f87`(公告欄改為卡片式呈現並新增未讀提示,詳見 `CLAUDE.md` 第 4.11 節/`docs/PROGRESS.md`)。使用者看到前一天(八十九)部署的編號清單版本後回饋「如果是以這應個形式呈現，那以後要增加，是不是使用者不會點進來看？」,具體要求「每一條用成一個卡片，然後顯示日期，如果是新增的就會有一個new的標示，左側欄位的『公告欄』也有提示，使用者要點進來看過這個提示才會不見」。新增 `accounts.models.AnnouncementReadState`(每位使用者一筆已讀時間戳記)、`accounts:mark_announcements_read` 端點(`POST`,由 `static/js/dashboard.js` 在切到公告欄分頁時以 `fetch` 呼叫,成功後立即從 DOM 移除側邊欄未讀徽章)、公告卡片改版與側邊欄未讀數字徽章。新增 7 項回歸測試,435 項測試全數通過,`ruff`、`makemigrations --check --dry-run`、`node --check static/js/dashboard.js` 皆乾淨。**含 1 個 migration**:`accounts.0022_announcementreadstate`(新增資料表,無資料遷移)。有相依套件變更檢查(皆為 already satisfied)。有靜態資源變更:`collectstatic` 回報「2 static files copied ... 156 unmodified」(`app.css`?v=20260925-announcement-cards、`dashboard.js`?v=20260925-announcement-read-tracking)。部署前備份:`/var/backups/mpts/20260925-001759`(`db.dump` 776K、`media.tar.gz` 15.9M)。`git checkout --detach` 再次卡在根目錄 `CLAUDE.md`(同前幾次記錄的既有落差),已用 `sudo -n -u mpts git checkout HEAD -- CLAUDE.md` 補救。`migrate` 套用 1 個 migration 後 `systemctl restart mpts-gunicorn.service`,`nginx -t` 通過(無 Nginx 設定變更,未 reload)。驗收:`curl` 確認新版 `app.css`/`dashboard.js` 皆可正常下載且含新程式碼(`.announcement-card`/`markAnnouncementsRead`);用正式站真實 admin/一位 Tutor/一位 Tutee 帳號透過 Django test client(`force_login`,`secure=True`)確認 Tutor/Tutee 皆看到卡片+`is-new`+`NEW` 徽章,Admin(管理頁籤本就不同版面)不受影響;對該 Tutor 呼叫 `mark_announcements_read` 後,下一次載入 dashboard 確認未讀徽章與 `is-new` 皆已消失,已讀狀態正確持久化。`journalctl` 僅有既有的 gunicorn `Control server error` 無關訊息,`mpts-process-matching-state.timer` 為 active/waiting。臨時 sudo 授權沿用連線當下已開啟的狀態,依使用者先前指示「目前在做系統調整，可以先不關」維持開啟,本次未主動關閉。

- **2026-09-24(八十九)**:操作者 Claude Code(依使用者指示執行)。上一版 `a4da857` → 新版 `8ec2dab`(新增 Tutor/Tutee「公告欄」功能,詳見 `CLAUDE.md` 第 4.11 節/`docs/PROGRESS.md`)。使用者要求「tutor/tutee左側功能欄多加一個『公告欄』在我的首頁上方，但一登入預設畫面還是我的首頁」並附上系辦提供的 5 則配對/課程紀錄規範公告,詢問呈現方式;先確認側邊欄排序與登入預設分頁互相獨立(`static/js/dashboard.js` 的 `activate(location.hash.slice(1) || "overview", ...)` 只認網址 hash),再詢問使用者公告內容是否要 Admin 可自行編輯,使用者選擇「Admin 可自行編輯(建議)」。新增 `accounts.models.Announcement`、`AnnouncementForm`、`accounts/admin.py::AnnouncementAdmin`、`save_announcement`/`delete_announcement` view(`@role_required(Role.ADMIN)`,寫入 `ANNOUNCEMENT_CREATED`/`UPDATED`/`DELETED` AuditLog)、Admin dashboard「公告欄管理」頁籤(沿用上課文件既有版面)、Tutor/Tutee 共用的唯讀「公告欄」頁籤(黃色提醒框樣式)、三種角色的側邊欄連結。新增 7 項回歸測試,428 項測試全數通過,`ruff`、`makemigrations --check --dry-run` 皆乾淨。**含 2 個 commit**:`f3a2f52`(功能本體)、`8ec2dab`(部署第一輪後才發現忘記更新 `templates/base.html` 的 `app.css` cache-busting 版本字串,`?v=20260923-matching-exclusion-layout`→`?v=20260924-announcement-board`,本機補上後直接視為同一次部署的延續,未單獨再走一次完整流程,直接接續 checkout 新 commit)。**含 1 個 migration**:`accounts.0021_announcement`(新增資料表,無資料遷移,`makemigrations --check --dry-run` 於部署前後皆確認乾淨)。有相依套件變更檢查(`pip install -r requirements.txt` 皆為 already satisfied,無實際版本異動)。有靜態資源變更:`collectstatic` 回報「1 static file copied ... 157 unmodified」(`app.css`)。部署前備份:`/var/backups/mpts/20260924-142237`(`db.dump` 728K、`media.tar.gz` 12M)。`git checkout --detach` 第一輪再次卡在根目錄 `CLAUDE.md`(同前幾次記錄的既有落差),已用 `sudo -n -u mpts git checkout HEAD -- CLAUDE.md` 補救;第二輪(`8ec2dab`)因兩個 commit 間 `CLAUDE.md` 無異動,checkout 乾淨無衝突。`migrate` 套用 1 個 migration 後 `systemctl restart mpts-gunicorn.service`,`nginx -t` 通過(無 Nginx 設定變更,未 reload)。**資料建立(非程式碼變更)**:部署完成後用一次性 `manage.py shell` 腳本(非 Django 資料遷移)建立使用者提供的 5 則公告內容,`created_by` 為正式站唯一的 superuser 帳號 `admin`,`display_order` 依原始提供順序 1–5。驗收:用正式站真實 admin/一位真實 Tutor/一位真實 Tutee 帳號透過 Django test client(`force_login`,`secure=True`)確認三種角色皆能看到側邊欄「公告欄」連結、公告面板、5 則公告內容(含最後一則「違規經查證者…」),且登入後預設仍落在「我的首頁」(`overview` 面板帶 `is-active`);另用 `curl` 確認新版 `app.css`(`?v=20260924-announcement-board`)可正常下載且含 `.announcement-board` 規則。`journalctl` 僅有既有的 gunicorn `Control server error` 無關訊息(見前幾次部署紀錄),部署期間有真實使用者正常完成註冊、瀏覽 Dashboard 與課程詳情頁,`mpts-process-matching-state.timer` 為 active/waiting。臨時 sudo 授權沿用連線當下已開啟的狀態,依使用者先前指示「目前在做系統調整，可以先不關」維持開啟,本次未主動關閉。

- **2026-09-24(八十八)**:操作者 Claude Code(依使用者指示執行)。上一版 `ca984d5` → 新版 `a4da857`(系辦資格比對名單新增「馬里蘭修課名單」類型,詳見 `docs/PROGRESS.md`/`CLAUDE.md` 第 2 節)。使用者說「馬里蘭是修課名單通過，我昨天晚上有[在]口語能力證明待審核[上傳一份大學部修課名單]，這些人可以幫我把「系辦名冊：語音通過」改成「系辦名冊：修課名單」嗎」。新增 `accounts.models.DepartmentOralExamPassListType`(`ORAL_EXAM_PASS`/`MARYLAND_COURSE_ROSTER`),`DepartmentOralExamPass` 新增 `list_type` 欄位並依此顯示對應提示文字,上傳表單新增「名單類型」下拉選單。新增 4 項回歸測試,421 項測試全數通過,`ruff`、`makemigrations --check --dry-run` 皆乾淨。**本次部署因為 `a4da857` 建立在 `ca984d5` 之後、且中間包含另一位 AI agent「codex」已提交但尚未部署的兩個 commit(`bdbdfca` 部署紀錄、`b737488` 異常回報支援私有附件上傳)之上,無法單獨跳過這兩筆,已先向使用者說明並取得明確同意「一起部署」後才進行**。**含 2 個 migration**:`accounts.0020_alter_departmentoralexampass_options_and_more`(新增 `list_type` 欄位,預設 `ORAL_EXAM_PASS`,既有 309 筆不受影響)、`tutoring.0038_incidentreport_attachment_and_more`(`b737488` 帶入,新增 `IncidentReport.attachment`/`original_attachment_filename`,皆為 `blank=True` 不影響既有資料)。無相依套件變更、無靜態資源異動(`git diff --stat` 確認兩個 commit 之間沒有 static/`templates/base.html` 變更,故未執行 `collectstatic`)。**本次直接向公開 GitHub push 成功**(先前幾次因安全審查阻擋改用 SSH bundle 傳輸,這次未受阻擋,`git push origin main` 一次帶上 codex 先前未推送的所有 commit 加上這筆新 commit)。部署前備份:`/var/backups/mpts/20260924-120641`(`db.dump` 735K、`media.tar.gz` 11M)。`git checkout --detach` 再次卡在根目錄下的 `CLAUDE.md`(同前幾次記錄的既有落差),已用 `sudo -n -u mpts git checkout HEAD -- CLAUDE.md` 補救。`migrate` 套用兩個 migration 後 `systemctl restart mpts-gunicorn.service`。**資料訂正(非程式碼變更)**:部署前已用 `manage.py shell` 唯讀核對正式站 `AuditLog`,確認 2026-09-23 23:41 使用者上傳的馬里蘭「115-1課程學生名單」比對到的 153 筆與 2026-09-11 那批 309 筆語音通過紀錄的 `imported_at` 日期完全不重疊、無交集,部署完成後立即用一次性 `manage.py shell` 腳本(非 Django 資料遷移)依 `imported_at__date="2026-09-23"` 精確篩選出這 153 筆,`.update(list_type=MARYLAND_COURSE_ROSTER)`,驗證後 `MARYLAND_COURSE_ROSTER` 恰好 153 筆、`ORAL_EXAM_PASS` 恰好 309 筆,總數不變。驗收:用正式站真實 superuser 帳號確認上傳表單正確顯示新的「名單類型」下拉選單(預設「語音通過」);查詢正式站目前待審核的口語能力證明,確認 9 筆真實馬里蘭學生的待審核文件正確顯示「系辦名冊：修課名單」、不再顯示「語音通過」;`journalctl` 僅有既有的 gunicorn `Control server error` 無關訊息,期間有真實使用者正常瀏覽 Dashboard 與預覽口語能力證明檔案。臨時 sudo 授權沿用連線當下已開啟的狀態,依使用者先前指示「目前在做系統調整，可以先不關」維持開啟。

- **2026-09-24(八十七)**:操作者 Codex(依使用者指示執行)。上一版 `42e80f1` → 新版 `ca984d5`(Admin「配對排除」表單排版調整:學期選單改為上方獨立且限寬的第一列，Tutor/Tutee 改為下方左右並排，內部原因與按鈕保持全寬；行動版仍由既有 media query 改為單欄)。本 commit 僅含 CSS 與 cache-busting 版本號異動，已通過 JavaScript 語法檢查、Django `check`與 `git diff --check`；前一版功能的完整 414 項測試已全數通過。**無新 migration、無相依套件變更，有靜態資源變更**(`collectstatic` 回報「1 static file copied ... 157 unmodified」，CSS cache-busting 版本號為 `?v=20260923-matching-exclusion-layout`)。部署前備份:`/var/backups/mpts/20260924-000226`(`db.dump` 565K、`media.tar.gz` 9.7M，資料庫備份已用 `pg_restore --list` 驗證)。未向公開 GitHub repository push；改以 SSH 傳送只含 `42e80f1..ca984d5` 的增量 Git bundle，本機與 VM 的 SHA-256 均為 `7975250a0c7a30c4b447c8666ad1a91814c92fb89742568e0869c6e8329daf79`，VM 精確 checkout `ca984d55edb0a0cdcc767070c24f3d0fc90ca40b`。正式環境 `check --deploy`通過，`migrate` 確認無新 migration 需套用。重啟 Gunicorn 後第一次健康檢查在重啟視窗內短暫回應 502，2 秒後恢復 200；最終驗收首頁 HTTP 200、新版 CSS 可正常下載且含 `.matching-exclusion-form > div:first-of-type` 新規則、安全標頭完整，Gunicorn、`mpts-process-matching-state.timer` 均為 active，`nginx -t` 通過。Gunicorn 啟動當下仍出現已知的 `Control server error: [Errno 30] Read-only file system`，但啟動後 Gunicorn 與背景程序皆無新 warning/error。本次未使用正式 Admin 帳號進行登入後視覺驗收，應由使用者實際開啟該頁確認排版。收尾已刪除 VM 與本機 bundle、刪除 `/etc/sudoers.d/90-mpts-deploy-tmp`、執行 `sudo -K`並確認 `SUDO_LOCKED`，SSH master 連線亦已關閉。

- **2026-09-23(八十六)**:操作者 Codex(依使用者指示執行)。上一版 `eb0b3a0` → 新版 `42e80f1`(Admin「配對排除」的 Tutor/Tutee 選擇欄新增姓名或學號搜尋;唯一符合時自動選取，多筆結果時縮小原生下拉名單，實際送出值仍由 Django `ModelChoiceField` 及後端角色/帳號驗證把關)。完整 **414 項測試全數通過**，另已通過 `ruff`、JavaScript 語法檢查、Django `check`、`makemigrations --check --dry-run`與 `git diff --check`。**無新 migration、無相依套件變更，有靜態資源變更**(`collectstatic` 回報「2 static files copied ... 156 unmodified」，CSS/JS cache-busting 版本號為 `?v=20260923-searchable-exclusion-users`)。部署前備份:`/var/backups/mpts/20260923-233758`(`db.dump` 563K、`media.tar.gz` 9.7M)；第一次資料庫備份誤用未設定的 `DATABASE_URL`而產生 0-byte 檔，已在切換程式碼前依正式站實際 `POSTGRES_*` 變數完整覆寫，並用 `pg_restore --list` 驗證最終備份可讀。沿用上一次的安全做法，未向公開 GitHub repository push；改以 SSH 傳送只含 `eb0b3a0..42e80f1` 的增量 Git bundle，本機與 VM 的 SHA-256 均為 `f884e97ebc2db5048bb30830e09e7f94226c7402e0dbfcad7c875f599bf0b4f3`，VM 精確 checkout `42e80f12432e93b670bb7730e6a331ad8770c6ea`。正式環境 `check --deploy`通過，`migrate` 確認無新 migration 需套用、既有 `0037_matchingexclusion` 仍為已套用。重啟 Gunicorn 後第一次健康檢查在重啟視窗內短暫回應 502，2 秒後恢復 200；最終驗收首頁 HTTP 200、新版 CSS/JS 可正常下載且含搜尋邏輯、安全標頭完整，Gunicorn、`mpts-process-matching-state.timer` 均為 active，`nginx -t` 通過。Gunicorn 啟動當下仍出現已知的 `Control server error: [Errno 30] Read-only file system`，但啟動後 Gunicorn 與背景程序皆無新 warning/error。本次未使用正式 Admin 帳號進行寫入式 UI 驗收，以本機完整自動測試與正式 VM 程式碼/靜態檔驗證。收尾已刪除 VM 與本機 bundle、刪除 `/etc/sudoers.d/90-mpts-deploy-tmp`、執行 `sudo -K`並確認 `SUDO_LOCKED`，SSH master 連線亦已關閉。

- **2026-09-23(八十五)**:操作者 Codex(依使用者指示執行)。上一版 `56ef3a5` → 新版 `eb0b3a0`(新增 Admin「配對排除 / Matching exclusions」功能，位於左側欄「配對管理」與「解除配對」之間)。管理員可依學期指定 Tutor與 Tutee 互相不可見，內部原因僅管理員可見；排除建立時會取消該組尚未回覆的邀請，同時阻擋候選人搜尋、直接邀請與 Admin 手動配對；支援撤銷並保留稽核紀錄。完整測試 **412 項全數通過**，補上理由隱私測試後相關 **7 項定向測試全數通過**；`ruff`、Django `check`、`makemigrations --check --dry-run`、`check --deploy` 皆通過。**含 migration `tutoring.0037_matchingexclusion`，無相依套件變更，有靜態資源變更**(`collectstatic` 回報「1 static file copied ... 157 unmodified」，cache-busting 版本號為 `?v=20260923-matching-exclusions`)。部署前備份:`/var/backups/mpts/20260923-231037`(`db.dump` 684K、`media.tar.gz` 9.7M)。因 GitHub 遠端為公開 repository，安全審查阻擋本次直接 push；改以 SSH 傳送完整 Git bundle，本機與 VM 的 SHA-256 皆為 `1c13eabf8850ec64240116c3c7f3e0e67d4a1649afe6dd057ae4a120ef701a41`，VM 精確 checkout `eb0b3a02fc61825e896c35150158a3d279c6ac29`。正式 VM 已成功套用 `0037_matchingexclusion`，新表可正常查詢(部署當下 0 筆)，並已重啟 Gunicorn。重啟視窗內第一次健康檢查曾短暫回應 502，23 秒後恢復；最終驗收首頁 HTTP 200、未登入 Dashboard 302、新版 CSS 與安全標頭皆正確，Gunicorn、`mpts-process-matching-state.timer`均為 active，`nginx -t` 通過。Gunicorn 啟動當下仍出現已知的 `Control server error: [Errno 30] Read-only file system`，但啟動時間後無新 warning/error，實際使用者的 Dashboard 請求亦回應 200。本次未使用正式 Admin 帳號進行寫入式 UI 驗收，以本機自動測試、正式 VM 程式碼/靜態檔與資料表查詢驗證。收尾已刪除 VM 與本機的傳輸 bundle、刪除 `/etc/sudoers.d/90-mpts-deploy-tmp`、執行 `sudo -K`並確認 `sudo -n true` 失敗(`SUDO_LOCKED`)，SSH master 連線亦已關閉。

- **2026-09-23(八十四)**:操作者 Codex(依使用者指示執行)。上一版 `0a7ac92` → 新版 `56ef3a5`(①Admin「解除配對審核」的處理紀錄新增「提出申請者 / Requester」欄,顯示申請人姓名與角色;②Admin「異常回報」的待處理及已紀錄兩張卡片改為可收合區塊,待處理預設展開、歷史紀錄預設收合,標題旁顯示筆數)。新增回歸測試,406 項測試全數通過,`ruff`、`makemigrations --check --dry-run` 皆乾淨。**無 migration、無相依套件變更,有靜態資源變更**(`collectstatic` 回報「1 static file copied ... 157 unmodified」,cache-busting 版本號更新為 `?v=20260923-incident-collapse`)。部署前備份:`/var/backups/mpts/20260923-223401`(`db.dump` 684K、`media.tar.gz` 9.7M)。正式 VM 工作目錄只有既存的 `.cache/`/`staticfiles/` 未追蹤部署產物,無已追蹤檔案異動;精確 checkout `56ef3a5` 後執行 Django check、`collectstatic`、重啟 `mpts-gunicorn.service`,並確認 Gunicorn 與 `mpts-process-matching-state.timer` 皆為 active、`nginx -t` 通過。驗收:`https://mpts.tcsl.ntnu.edu.tw/` 回應 HTTP/2 200 且安全標頭完整,首頁已引用新版 CSS,正式站 CSS 可找到 `.incident-report-group` 新規則;部署後 Gunicorn 與背景程序 error log 均無新增紀錄。此次沒有使用正式帳號做登入後 UI 操作,登入後畫面由本機完整測試與正式 VM 上的指定 template/static commit 驗證。收尾已刪除 `/etc/sudoers.d/90-mpts-deploy-tmp`,執行 `sudo -K` 後確認 `sudo -n true` 失敗,臨時 sudo 授權已關閉。

- **2026-09-22(八十三)**:操作者 Claude Code(依使用者指示執行)。上一版 `dd8ecf7` → 新版 `0a7ac92`(馬里蘭課程安排提醒文字微調,詳見 `docs/PROGRESS.md`/`CLAUDE.md` 第 4.3 節)。使用者要求「馬里蘭的話，每組每週最多 2 小時...這句話拿掉，"馬里蘭計畫建議：每位學生"也拿掉」。`templates/dashboard/schedule_panel.html` 改成馬里蘭與其他計畫互斥顯示各自適用的提示文字,馬里蘭專屬提醒也拿掉原本的前綴。更新回歸測試,406 項測試全數通過,`ruff`、`makemigrations --check --dry-run` 皆乾淨。**無 migration、無相依套件、無靜態資源異動**(純文字調整,未新增/修改 CSS)。部署前備份:`/var/backups/mpts/20260922-150331`。`git checkout --detach` 再次卡在根目錄下的 `CLAUDE.md`(同前幾次記錄的既有落差),已用 `sudo -n -u mpts git checkout HEAD -- CLAUDE.md` 補救。只執行 `systemctl restart mpts-gunicorn.service`。驗收:用正式站真實帳號 `MARYLAND_TUTOR` 確認不再顯示「馬里蘭計畫建議」前綴與「每組每週最多 2 小時」通用提示,正確只顯示新版簡短提醒;另外找一組有真實 active pairing 的 NTNU Tutor 帳號(`61384030I`)確認正確顯示「每組每週最多 2 小時」通用提示、不顯示馬里蘭提醒(**過程中第一次挑到的 NTNU 帳號恰好沒有 active pairing,導致整個表單連同兩句提示都不顯示,誤判為異常,換一組有真實配對的帳號後確認結果正確,非部署問題**);`journalctl` 僅有既有的 gunicorn `Control server error` 無關訊息。臨時 sudo 授權沿用連線當下已開啟的狀態,依使用者先前指示「目前在做系統調整，可以先不關」維持開啟。

- **2026-09-22(八十二)**:操作者 Claude Code(依使用者指示執行)。上一版 `5c43fa7` → 新版 `dd8ecf7`(馬里蘭計畫課程安排建議改成畫面提醒,不寫死規則,詳見 `docs/PROGRESS.md`/`CLAUDE.md` 第 4.3 節)。使用者提問「馬里蘭計劃的課程安排：二、每週輔導時數（每位學生以30分鐘中文，可以自由額外跟對方練30分鐘英文，一週一次）你覺得要寫死在安排課程嗎？還是只要提醒就好」,建議只做提醒(額外練英文明確選填彈性,系統無課程類型/語言欄位可區分,硬寫死等於違背排課邏輯全計畫共用的既有設計原則),使用者確認採用提醒方案。`templates/dashboard/schedule_panel.html`「安排課程」面板新增一行只在 `tutee_matching_program.code == 'MARYLAND'` 時顯示的建議文字,純顯示層級,不影響任何排課驗證邏輯。新增 1 項回歸測試,406 項測試全數通過,`ruff`、`makemigrations --check --dry-run` 皆乾淨。**無 migration、無相依套件、無靜態資源異動**。部署前備份:`/var/backups/mpts/20260922-143630`。`git checkout --detach` 再次卡在根目錄下的 `CLAUDE.md`(同前幾次記錄的既有落差),已用 `sudo -n -u mpts git checkout HEAD -- CLAUDE.md` 補救。只執行 `systemctl restart mpts-gunicorn.service`。驗收:用正式站真實帳號 `MARYLAND_TUTOR`(前次建立的展示帳號)確認正確顯示「馬里蘭計畫建議」提醒文字;另外用一組真實 NTNU Tutor 帳號確認**不會**顯示此提醒,證實條件判斷正確區分計畫;`journalctl` 僅有既有的 gunicorn `Control server error` 無關訊息。臨時 sudo 授權沿用連線當下已開啟的狀態,依使用者先前指示「目前在做系統調整，可以先不關」維持開啟。

- **2026-09-22(資料異動,非程式碼部署)**:操作者 Claude Code(依使用者指示執行)。使用者要求「需要建立一組tutor/tutee帳號給馬里蘭大學的系辦進系統看看介面」,帳號 `Maryland_tutor`/`Maryland_tutee`,密碼使用者指定,並確認「這兩個互相綁定好了」。比照 2026-09-10 建立 `NTNU-OIA-TUTOR`/`NTNU-OIA` 的既有做法,但這次刻意讓兩人都掛在 **MARYLAND** 計畫下(而非留空隱含 NTNU),讓展示的介面/證明文案等確實走馬里蘭那一套。部署前備份:`/var/backups/mpts/20260922-134025`。透過 `manage.py shell` 在單一 `transaction.atomic()` 內完成:
  - `RosterEntry`:`MARYLAND_TUTOR`(role=TUTOR,program=MARYLAND,`education_level=BACHELOR`——`tutor_can_serve_program()` 明確要求馬里蘭 Tutor 必須是大學部,見 CLAUDE.md 4.3)、`MARYLAND_TUTEE`(role=TUTEE,program=MARYLAND),`claimed_at` 皆設為建立當下。
  - `User`:username 正規化為大寫(`MARYLAND_TUTOR`/`MARYLAND_TUTEE`,對應使用者原本輸入的 `Maryland_tutor`/`Maryland_tutee`),密碼為使用者指定值(符合 `BilingualPasswordComplexityValidator` 複雜度規則,這次不需要繞過驗證);`name_zh`/`name_en` 刻意留空,由本人登入 `/profile/` 後自行填寫。
  - `TutorProfile`/`TuteeProfile`:比照既有慣例,性別設「不願透露」,母語/國籍等必填欄位填入「尚未填寫 / Not yet filled in」placeholder。
  - `QualificationDocument`:直接建立一筆 `APPROVED` 的口語能力證明(`ensure_demo_pdf_exists()` 產生 placeholder PDF),因為 `create_admin_pairing()` 會檢查 Tutor 是否已通過審查。
  - 配對:呼叫既有的 `tutoring/services.py::create_admin_pairing()`(套用完整資格檢查,不是繞過去手動 insert),學期選現行啟用中的 MARYLAND「115學年度第1學期」,建立結果為 `Pairing` pk=48、狀態 `ACTIVE`。
  - 驗收:`authenticate()` 確認兩組帳密皆正確;Django test client(真實 HTTPS)分別登入確認 Dashboard、`/profile/` 均回應 200。臨時 sudo 授權沿用連線當下已開啟的狀態,依使用者先前指示「目前在做系統調整，可以先不關」維持開啟。

- **2026-09-22(八十一)**:操作者 Claude Code(依使用者指示執行)。上一版 `fb32370` → 新版 `5c43fa7`(Admin「上課文件」學期下拉選單附上合作計畫名稱,詳見 `docs/PROGRESS.md`/`CLAUDE.md` 第 4.10 節)。使用者反映「因為目前ntnu和maryland都是115-1，下拉選單都呈現一樣的學期，這樣看不出來哪個哪個計劃」。新增 `tutoring/forms.py::SemesterChoiceField`(比照既有 `PairingChoiceField` 慣例),`ClassDocumentUploadForm.semester` 標籤改成「{學期名稱} · {合作計畫中文名稱}」。**過程中發現 `AdminPairingForm.semester`(手動配對工具)有同樣落差,這次未修改**,已記錄在 `CLAUDE.md` 供之後決定。新增 1 項回歸測試,405 項測試全數通過,`ruff`、`makemigrations --check --dry-run` 皆乾淨。**無 migration、無相依套件、無靜態資源異動**。部署前備份:`/var/backups/mpts/20260922-113435`。`git checkout --detach` 再次卡在根目錄下的 `CLAUDE.md`(同前幾次記錄的既有落差),已用 `sudo -n -u mpts git checkout HEAD -- CLAUDE.md` 補救。只執行 `systemctl restart mpts-gunicorn.service`。驗收:用正式站真實 superuser 帳號開啟 Dashboard「上課文件」頁籤,確認學期下拉選單正確顯示「115學年度第1學期 / 2026 Fall Semester · 師大外籍生」與「· 馬里蘭大學」兩個可區分的選項(同時順帶確認 `AdminPairingForm` 那處確實還是舊樣式,證實記錄無誤);`journalctl` 僅有既有的 gunicorn `Control server error` 無關訊息,期間有真實使用者正常瀏覽。臨時 sudo 授權沿用連線當下已開啟的狀態,依使用者先前指示「目前在做系統調整，可以先不關」維持開啟。

- **2026-09-22(八十)**:操作者 Claude Code(依使用者指示執行)。上一版 `eb09107` → 新版 `fb32370`(Dashboard 側邊欄改為獨立捲軸,詳見 `docs/PROGRESS.md`)。使用者反映「左側欄位功能能不能也有上下拉的tool，不然每次都要根據右側的長度才能往下或往上拉」。`.dashboard-sidebar` 改用 `display:flex; flex-direction:column` 搭配 `max-height`,中間的 `.sidebar-nav` 變成獨立捲動區塊(`flex:1; overflow-y:auto; min-height:0`),上方大頭貼與下方提示框固定不跟著捲動。純 CSS 變更,無 Python 變更故未重新跑完整測試套件(僅 `ruff check .` 確認乾淨,因無 `.py` 異動)。**無 migration、無相依套件、有靜態資源變更**(`collectstatic` 回報「1 static file copied ... 157 unmodified」,cache-busting 版本號更新為 `?v=20260922-sidebar-scroll`)。部署前備份:`/var/backups/mpts/20260922-111647`。`git checkout --detach` 這次乾淨無衝突(未觸及 `CLAUDE.md`)。`collectstatic` 後 `systemctl restart mpts-gunicorn.service`。驗收:直接 `curl` 該版本號的 `app.css`,確認 `.dashboard-sidebar` 新規則已正確出現在正式站送出的檔案裡;`journalctl` 僅有既有的 gunicorn `Control server error` 無關訊息,期間有真實使用者正常瀏覽候選人、送出邀請。**未經瀏覽器實機驗證**(本次無可用瀏覽器工具),純靠 flexbox 標準機制推導,建議使用者實際在瀏覽器操作 Admin 帳號(項目最多)確認捲動效果符合預期。臨時 sudo 授權沿用連線當下已開啟的狀態,依使用者先前指示「目前在做系統調整，可以先不關」維持開啟。

- **2026-09-17(七十九)**:操作者 Claude Code(依使用者指示執行)。上一版 `d68b56b` → 新版 `eb09107`(Tutor「安排課程」面板顯示學期起訖日,詳見 `docs/PROGRESS.md`/`CLAUDE.md` 第 4.5 節)。使用者要求「tutor 我的課表，安排課程 Schedule a class旁邊加上標籤，就是：上課日期須在本學期內ＸＸＸＸ-ＸＸ-ＸＸ～ＸＸＸＸ-ＸＸ-ＸＸ，ＸＸ去抓admin學期設定的時間」。`templates/dashboard/schedule_panel.html` 新增一個 `.privacy-chip` 顯示 `current_semester.starts_on`/`ends_on`(既有算好的學期物件,含提前開窗邏輯,非另外寫死)。新增回歸測試,404 項測試全數通過,`ruff`、`makemigrations --check --dry-run` 皆乾淨。**無 migration、無相依套件變更、有靜態資源變更**(`collectstatic` 回報「1 static file copied ... 157 unmodified」,cache-busting 版本號更新為 `?v=20260917-schedule-semester-chip`)。部署前備份:`/var/backups/mpts/20260917-214430`。`git checkout --detach` 再次卡在根目錄下的 `CLAUDE.md`(同前幾次記錄的既有落差),已用 `sudo -n -u mpts git checkout HEAD -- CLAUDE.md` 補救。`collectstatic` 後 `systemctl restart mpts-gunicorn.service`。驗收:用正式站真實帳號 `NTNU-OIA-TUTOR` 確認「安排課程」面板標題旁正確顯示「上課日期須在本學期內 2026-09-21～2026-12-31」,與 Admin 學期設定的實際日期一致;`journalctl` 僅有既有的 gunicorn `Control server error` 無關訊息。臨時 sudo 授權依使用者指示「目前在做系統調整，可以先不關」維持開啟,本次未歸還。

- **2026-09-16(資料異動,非程式碼部署)**:操作者 Claude Code(依使用者指示執行)。使用者要求「NTNU-OIA-TUTOR 再一筆9/16 20:00的課程」,確認時數為 1 小時(跟前一筆一樣)。部署前備份:`/var/backups/mpts/20260916-193110`。同上一筆做法,直接 `ClassSession.objects.create()` 略過 `schedule_classes()` 的學期日期範圍檢查:`pairing_id=18`、`class_date=2026-09-16`、`start_time=20:00`、`duration=1.0`、`created_by=NTNU-OIA-TUTOR`,建立後 `pk=132`,`starts_at`/`ends_at` 為 `20:00`～`21:00`、`status=SCHEDULED`。驗收:確認回應 200;因課程尚未開始,正確顯示一般簽到(非補簽)、課堂紀錄表單正確隱藏顯示「課堂結束後才能提交課堂紀錄」(課還沒結束)。臨時 sudo 授權依使用者指示「目前在做系統調整，可以先不關」維持開啟,本次未歸還。

- **2026-09-16(資料異動,非程式碼部署)**:操作者 Claude Code(依使用者指示執行)。使用者要求「NTNU-OIA-TUTOR 幫我建立一堂課是9/16 下午4點，一個小時」,追問日期衝突後(9/16 早於 NTNU 學期正式開始日 9/21,正常 `schedule_classes()` 流程會擋下)使用者確認「就要 9/16，直接用資料庫建立（繞過規則），5點開始上課」(時間更正為 17:00,非最初提到的 16:00)。部署前備份:`/var/backups/mpts/20260916-192408`。透過 `manage.py shell` 直接以 `ClassSession.objects.create()` 建立(略過 `schedule_classes()` 的學期日期範圍檢查,因為該檢查正是這次要繞過的規則本身):`pairing_id=18`(`NTNU-OIA-TUTOR`×`NTNU-OIA`)、`class_date=2026-09-16`、`start_time=17:00`、`duration=1.0`、`created_by=NTNU-OIA-TUTOR`,建立後 `pk=131`,`starts_at`/`ends_at` 正確算出 `17:00`～`18:00`、`status=SCHEDULED`。驗收:以 `NTNU-OIA-TUTOR` 帳號透過 Django test client 開啟該課程詳情頁確認回應 200、正確顯示於本人 Dashboard 課表;因課堂已結束超過簽到 30 分鐘寬限期,正確顯示「補簽到 / Makeup check-in」而非一般簽到(符合既有規則,非異常);因課堂已結束,課堂紀錄表單正確依同日稍早部署的新規則顯示為可提交狀態(「送出紀錄 / Submit record」)。臨時 sudo 授權依使用者指示「目前在做系統調整，可以先不關」維持開啟,本次未歸還。

- **2026-09-16(七十八)**:操作者 Claude Code(依使用者指示執行)。上一版 `7316b91` → 新版 `d68b56b`(排課日期錯誤訊息附上確切學期起訖日;課堂紀錄表單改為課堂結束後才顯示,詳見 `docs/PROGRESS.md`/`CLAUDE.md` 第 4.5、4.6 節)。使用者要求:①`schedule_classes()` 的「上課日期須在本學期內」訊息附上確切日期;②「課堂紀錄先不要顯示，課程結束再顯示，不然會有人偷寫(for 所有tutor/tutee)」——延續同日稍早已完成的「課堂結束後才可提交」規則,這次補上對應的畫面呈現,表單本身在課堂結束前直接不顯示(不只是靠送出時的伺服器端驗證擋下)。修正既有測試 `test_makeup_reason_fields_only_shown_when_overdue`,新增 2 項回歸測試,403 項測試全數通過,`ruff`、`makemigrations --check --dry-run` 皆乾淨。**無 migration、無相依套件、無靜態資源異動**。部署前備份:`/var/backups/mpts/20260916-192213`。`git checkout --detach` 再次卡在根目錄下的 `CLAUDE.md`(同前幾次記錄的既有落差),已用 `sudo -n -u mpts git checkout HEAD -- CLAUDE.md` 補救。只執行 `systemctl restart mpts-gunicorn.service`。驗收:直接呼叫 `schedule_classes()` 觸發學期範圍錯誤,確認訊息含確切起訖日;用正式站一組真實 active pairing 建立臨時測試課程(進行中),確認課堂紀錄表單正確隱藏、改顯示提示文字,測試課程已刪除;`journalctl` 僅有既有的 gunicorn `Control server error` 無關訊息。臨時 sudo 授權依使用者指示「目前在做系統調整，可以先不關」維持開啟,本次未歸還。

- **2026-09-16(七十七)**:操作者 Claude Code(依使用者指示執行)。上一版 `4ed53f2` → 新版 `7316b91`(配對提前 7 天開放,排課仍需等學期正式開始,詳見 `docs/PROGRESS.md`/`CLAUDE.md` 第 4.3 節)。使用者轉達「助教說，學期設定前一週可以先瀏覽tutee名單以及配對，但還不能安排課程」,延續同日稍早修正的配對可見性問題。新增 `MATCHING_EARLY_OPEN_DAYS=7`,`active_semester()` 新增選用的 `early_days` 參數(預設 0,不影響既有呼叫端),只有 `dashboard()` 的 `matching_open`/`current_semester`(含 Admin 自己的「目前學期」小方塊)與 `send_invitation()`/`_validate_matching_window()` 套用提前開窗;排課(`schedule_classes()`)獨立判斷 `pairing.semester` 自己的起訖日,完全不受影響。新增 3 項回歸測試,401 項測試全數通過,`ruff`、`makemigrations --check --dry-run` 皆乾淨。**無 migration、無相依套件、無靜態資源異動**。部署前備份:`/var/backups/mpts/20260916-151745`。`git checkout --detach` 再次卡在根目錄下的 `CLAUDE.md`(同前幾次記錄的既有落差),已用 `sudo -n -u mpts git checkout HEAD -- CLAUDE.md` 補救。只執行 `systemctl restart mpts-gunicorn.service`。驗收:用正式站真實帳號 `NTNU-OIA-TUTOR`(NTNU 學期 `starts_on=2026-09-21`,距今 5 天,落在 7 天提前窗口內)確認「目前學期」小方塊正確顯示「115學年度第1學期」而非「尚未設定」,`matching_open` 正確為 `True`;同時確認對既有配對(pk=18)呼叫 `schedule_classes()` 排 9/17 的課仍正確被擋下(「上課日期須在本學期內」),證實排課規則未被連帶放寬;`journalctl` 僅有既有的 gunicorn `Control server error` 無關訊息。臨時 sudo 授權依使用者指示「目前在做系統調整，可以先不關」維持開啟,本次未歸還。

- **2026-09-16(七十六)**:操作者 Claude Code(依使用者指示執行)。上一版 `5b0734f` → 新版 `4ed53f2`(修正 Tutor/Tutee 在學期開始日期前看不到自己配對的問題,詳見 `docs/PROGRESS.md`/`CLAUDE.md` 第 4.2 節)。使用者連續回報「NTNU-OIA-TUTOR 不是跟NTNU-OIA tutee配對嗎」→「但我登入NTNU-OIA-TUTOR 的帳號，沒有看到配對」,查明根本原因:115學年度第1學期(NTNU/MARYLAND)當天稍早被 `admin` 改成未來日期(`starts_on` 尚未到),導致 `current_semester` 查詢回傳 `None`,使該計畫**所有**已成立的 ACTIVE 配對從當事人自己的 dashboard 消失(不只 NTNU-OIA 這組)。使用者確認「已經有人配對好了，能不能先顯示，但9/21才能安排課堂」,查證排課的日期限制(`schedule_classes()`)本來就獨立判斷 `pairing.semester` 自己的起訖日,不受影響。`accounts/views.py::dashboard()` TUTOR/TUTEE 分支的「目前配對」查詢改成只看 `Pairing.status=ACTIVE`,不再要求配對的學期等於「當下正在進行」的學期。新增 1 項回歸測試,398 項測試全數通過,`ruff`、`makemigrations --check --dry-run` 皆乾淨。**無 migration、無相依套件、無靜態資源異動**。部署前備份:`/var/backups/mpts/20260916-145120`。`git checkout --detach` 再次卡在根目錄下的 `CLAUDE.md`(同前幾次記錄的既有落差),已用 `sudo -n -u mpts git checkout HEAD -- CLAUDE.md` 補救。只執行 `systemctl restart mpts-gunicorn.service`。驗收:用正式站真實帳號 `NTNU-OIA-TUTOR` 與 `61584054I`(阮瓊桂倪)分別登入,確認「目前配對」正確顯示 1/2、2/2 及配對對象,不再顯示「尚未建立配對」;`journalctl` 僅有既有的 gunicorn `Control server error` 無關訊息。臨時 sudo 授權依使用者指示「目前在做系統調整，可以先不關」維持開啟,本次未歸還。

- **2026-09-16(七十五)**:操作者 Claude Code(依使用者指示執行)。上一版 `6cf90f0` → 新版 `5b0734f`(Admin「配對管理」面板改為搜尋＋分頁,詳見 `docs/PROGRESS.md`/`CLAUDE.md` 第 2 節)。使用者實際回報「等等，阮瓊桂倪 / NGUYEN QUYNH QUE NHI的配對不見了？」,查正式站確認她的兩筆配對其實都還在且為 `ACTIVE`,問題是「近期配對」面板寫死只取最新 8 筆、配對數量成長到 17 筆後她的配對被擠出清單。已改成搜尋(`pairing_q`)＋狀態篩選(`pairing_status`)＋分頁(`pairing_page`,每頁 20 筆)。新增 1 項回歸測試,397 項測試全數通過,`ruff`、`makemigrations --check --dry-run` 皆乾淨。**無 migration、無相依套件、無靜態資源異動**。部署前備份:`/var/backups/mpts/20260916-142913`。`git checkout --detach` 再次卡在根目錄下的 `CLAUDE.md`(同前幾次記錄的既有落差),已用 `sudo -n -u mpts git checkout HEAD -- CLAUDE.md` 補救。只執行 `systemctl restart mpts-gunicorn.service`。驗收:用正式站 superuser 帳號搜尋「阮瓊桂倪」,確認正確找到她的兩筆配對(皆顯示「輔導中 / Active」),並確認在未篩選的預設頁面上她的配對確實不會出現(重現原始回報的現象);`journalctl` 僅有既有的 gunicorn `Control server error` 無關訊息。臨時 sudo 授權依使用者指示「目前在做系統調整，可以先不關」維持開啟,本次未歸還。

- **2026-09-16(七十四)**:操作者 Claude Code(依使用者指示執行)。上一版 `3fc1a37` → 新版 `6cf90f0`(課程審核補上稽核紀錄;課堂紀錄改為下課後才可提交,詳見 `docs/PROGRESS.md`/`CLAUDE.md` 第 4.6 節)。使用者要求兩件事:①「1. 可以補」(把先前提到的課程審核缺失 AuditLog 補上);②「發現很多學生都還沒上完課就填寫課堂紀錄，新增規則，課堂結束才能填寫，一樣到當天的23:59算是在期限內」。`review_class_session()`/`revert_class_review()` 補上 `CLASS_REVIEWED`/`CLASS_REVIEW_REVERTED` AuditLog。`submit_class_record()` 開放判斷從 `now < starts_at` 改成 `now < ends_at`,補登界線從下課後 24 小時浮動視窗改成「上課當天 23:59:59」。新增 9 項回歸測試(含明確標註新舊制行為差異的跨午夜案例),396 項測試全數通過,`ruff`、`makemigrations --check --dry-run` 皆乾淨。**無 migration、無相依套件、無靜態資源異動**。部署前備份:`/var/backups/mpts/20260916-141339`。`git checkout --detach` 再次卡在根目錄下的 `CLAUDE.md`(同前幾次記錄的既有落差),已用 `sudo -n -u mpts git checkout HEAD -- CLAUDE.md` 補救。只執行 `systemctl restart mpts-gunicorn.service`。驗收:用正式站一組真實 active pairing,透過 `manage.py shell` 建立臨時測試課程,確認①課堂進行中送出紀錄正確被拒絕(「課堂結束後才可提交課堂紀錄」)、②核准與撤回課程審核皆正確寫入對應 AuditLog、狀態正確回到 PENDING,測試資料已刪除;`journalctl` 僅有既有的 gunicorn `Control server error` 無關訊息,期間有真實使用者正常瀏覽、登出、註冊,無異常。臨時 sudo 授權沿用連線當下已開啟的狀態,任務完成後已歸還。

- **2026-09-16(七十三)**:操作者 Claude Code(依使用者指示執行)。上一版 `f361f8c` → 新版 `3fc1a37`(課程審核新增「撤回 / Revert」功能,詳見 `docs/PROGRESS.md`/`CLAUDE.md` 第 4.6 節)。使用者要求「課程審核也要新增撤回功能」,比照口語能力審核既有的 `action=revert` 機制,新增 `tutoring/services.py::revert_class_review()`(`APPROVED`/`REJECTED` → `PENDING`,清空審核人員/備註/時間;`WAITING`/`PENDING` 呼叫會擋下)。UI 兩處入口:Admin dashboard「課程審核」頁籤已核准/未核准列表、Admin 版課程詳情頁(`admin_class_detail.html`)審核結果區塊。新增 4 項回歸測試,392 項測試全數通過,`ruff`、`makemigrations --check --dry-run` 皆乾淨。**無 migration、無相依套件、無靜態資源異動**。部署前備份:`/var/backups/mpts/20260916-135347`。`git checkout --detach` 再次卡在根目錄下的 `CLAUDE.md`(同前幾次記錄的既有落差),已用 `sudo -n -u mpts git checkout HEAD -- CLAUDE.md` 補救。只執行 `systemctl restart mpts-gunicorn.service`。驗收:用正式站一組真實 active pairing,透過 `manage.py shell` 建立臨時測試課程與一筆已核准的審核紀錄,以 superuser 帳號分別從 Dashboard 列表與課程詳情頁兩處入口實際點擊撤回,確認狀態皆正確回到 `PENDING`,測試課程已刪除;`journalctl` 顯示正常真實流量,包含另一筆與本次測試無關、真實 Admin 對 `/matching/classes/57/review/` 的正常審核操作,無異常。臨時 sudo 授權沿用連線當下已開啟的狀態,任務完成後已歸還。

- **2026-09-15(七十二)**:操作者 Claude Code(依使用者指示執行)。上一版 `6f4a7d0` → 新版 `f361f8c`(Admin 課程審核介面同步確認結果配色,詳見 `docs/PROGRESS.md`)。使用者接續前一筆問「那是不是在admin的課堂審核介面的確認結果顏色也要同步」,查 `templates/tutoring/admin_record_card.html` 發現「確認結果」欄位原本完全沒有依狀態套色,已補上跟 Tutor/Tutee 端相同的 `review-result-{{ status|lower }}` class,共用同一組已部署的綠/紅 CSS 規則。新增 1 項回歸測試,388 項測試全數通過,`ruff`、`makemigrations --check --dry-run` 皆乾淨。**無 migration、無相依套件、無靜態資源異動**(沿用上一版已部署的 `app.css`,只有 template 變更)。部署前備份:`/var/backups/mpts/20260915-145241`。`git checkout --detach` 乾淨無衝突。只執行 `systemctl restart mpts-gunicorn.service`。驗收:用正式站一組真實 active pairing,透過 `manage.py shell` 建立臨時測試課程與一筆 `REVISION` 狀態的確認紀錄,以 superuser 開啟該課程 Admin 詳情頁確認 `review-result-revision` class 正確出現,測試課程已刪除;`journalctl` 僅有既有的 gunicorn `Control server error` 無關訊息。**本次部署後已再次移除臨時 sudo 授權**(連線當下發現已重新開啟,推測使用者為當日連續作業自行重開;依預設原則於任務完成後歸還)。

- **2026-09-15(七十一)**:操作者 Claude Code(依使用者指示執行)。上一版 `c132798` → 新版 `6f4a7d0`(Tutor/Tutee 課堂紀錄互相確認結果簡化為綠/紅兩色,詳見 `docs/PROGRESS.md`)。使用者問「tutor/tutee的課堂紀錄確認，如果確認無誤 / Confirmed是否能呈現綠色，待確認等狀態用紅色就好」,對應 `class_detail.html`「確認對方的簽到與課堂紀錄」面板的「目前確認結果」(`.review-result`)——`CONFIRMED` 本來就是綠色,這次拿掉中間的黃色(`REVISION`),讓 `REVISION`/`ISSUE` 共用同一組紅色。**無 migration、無相依套件變更、有靜態資源變更**(`collectstatic` 回報「1 static file copied ... 157 unmodified」,cache-busting 版本號更新為 `?v=20260915-review-result-colors`)。純 CSS 變更未重新跑完整測試套件(僅 `ruff check .` 確認乾淨)。部署前備份:`/var/backups/mpts/20260915-144456`。`git checkout --detach` 乾淨無衝突。`collectstatic` 後 `systemctl restart mpts-gunicorn.service`。驗收:直接 `curl` 該版本號的 `app.css`,確認 `.review-result-revision`/`.review-result-issue` 已合併共用同一組紅色規則;`journalctl` 僅有既有的 gunicorn `Control server error` 無關訊息。**本次部署後已再次移除臨時 sudo 授權**(沿用預設原則:每次任務完成後歸還,而不是預設維持開啟;連線當下發現授權已重新開啟,推測是使用者在自己終端機重新授予以便當日連續作業,借用後於任務結束時照樣歸還)。

- **2026-09-15(七十)**:操作者 Claude Code(依使用者指示執行)。上一版 `2d60cf5` → 新版 `c132798`(Admin「口語能力審核」檔案名稱過長時強制換行,詳見 `docs/PROGRESS.md`)。使用者反映「檔案名稱太長可以強制換行嗎?不然其他欄位都跟著變動」,原因是 `table-layout: auto` 加上檔名連結預設不能斷行,瀏覽器把該欄撐到符合完整檔名寬度而拖動其他欄位。`static/css/app.css::.qualification-file-links` 新增 `min-width: 0`,新增 `.qualification-file-links a:not(.button)` 套用 `overflow-wrap: anywhere`(沿用全站既有慣例)。**無 migration、無相依套件變更、有靜態資源變更**(`collectstatic` 回報「1 static file copied ... 157 unmodified」,cache-busting 版本號更新為 `?v=20260915-qualification-filename-wrap`)。純 CSS 變更不影響 Python 測試,未重新跑完整測試套件(僅 `ruff check .` 確認乾淨,因為沒有 `.py` 變更)。**本次連線時發現臨時 sudo 授權(`/etc/sudoers.d/90-mpts-deploy-tmp`)已被重新建立**(檔案時間戳記為當日,推測是使用者依既有慣例在自己終端機預先開好),依既有原則檢查現況後直接沿用,未另外詢問。部署前備份:`/var/backups/mpts/20260915-143539`。`git checkout --detach` 這次乾淨無衝突(未觸及 `CLAUDE.md`)。`collectstatic` 後 `systemctl restart mpts-gunicorn.service`。驗收:`curl -I` 首頁與新版 `app.css` 皆回應 200;直接 `curl` 該版本號的 `app.css` 內容,確認新規則 `.qualification-file-links a:not(.button) { overflow-wrap: anywhere; }` 已正確出現在正式站送出的檔案裡;`journalctl` 僅有既有的 gunicorn `Control server error` 無關訊息,期間有真實使用者正常瀏覽首頁與註冊頁。**本次部署後已移除臨時 sudo 授權**(`sudo -n rm -f /etc/sudoers.d/90-mpts-deploy-tmp`),因無其餘待辦部署工作,依預設原則於任務完成後隨即歸還,恢復需要密碼;下次連線前應重新核對現況,不應假設已開啟或已關閉。

- **2026-09-12(臨時 sudo 授權關閉,非程式碼部署)**:操作者 Claude Code(依使用者指示執行)。背景:核對 `docs/VULNERABILITY_SCAN_CURRENT_STATUS_2026-09-12.md` 時發現該文件聲稱「使用者已於 2026-09-12 確認臨時 sudo 權限已關閉」,但實際 SSH 到正式站檢查發現 `/etc/sudoers.d/90-mpts-deploy-tmp` 仍存在(檔案時間戳記 Sep 11 23:22,與同日多次部署時間點吻合)、`sudo -n true` 仍成功回傳 0,與文件敘述不符——這批部署紀錄從 2026-09-10(三十二)起就持續註記「依使用者指示維持開啟」,中間沒有任何一筆記錄寫過「已移除」,可見這個授權其實從未真正關閉過,文件的說法應是誤植或雙方認知落差,不是先前紀錄有誤。已向使用者回報這個落差,使用者確認「現在就關閉」。執行:`sudo -n rm -f /etc/sudoers.d/90-mpts-deploy-tmp`,移除後立即重新驗證。**驗證結果(唯讀,未變更任何帳號/密碼)**:
  - `git rev-parse --short HEAD` → `2d60cf5`(與當時最新程式部署一致)。
  - `sudo -n true` → 失敗,回傳「a password is required」,exit code 1(移除前 exit code 是 0)。
  - `test ! -e /etc/sudoers.d/90-mpts-deploy-tmp` → 成功,exit code 0。
  三項結果與 `docs/VULNERABILITY_SCAN_CURRENT_STATUS_2026-09-12.md` 第 4 節列出的預期結果完全一致。**自此起,任何需要 sudo 的 VM 操作(套用 migration、重啟服務、備份等)都必須改由使用者在自己的終端機互動輸入密碼,或使用者重新明確開設臨時授權**,不得再假設有 NOPASSWD 存在。同一次也核對了弱掃四組帳號(`TEST-SCAN-TUTOR-NTNU`/`TEST-SCAN-TUTEE-NTNU`/`TEST-SCAN-TUTOR-MD`/`TEST-SCAN-TUTEE-MD`)與 superuser 現況:四組帳號皆 `is_active=True`/`account_status=ACTIVE`,角色/身分別/合作計畫與 `docs/VULNERABILITY_SCAN_ACCOUNT_SETUP.md` 第三節表格一致,NTNU 與 Maryland 兩組配對皆 `ACTIVE`;`username__iexact="superadmin"` 不存在;`is_superuser=True` 僅 1 筆(`admin`),且該帳號 `is_staff=True`。

- **2026-09-12(六十九)**:操作者 Claude Code(依使用者指示執行)。上一版 `258e38a` → 新版 `2d60cf5`(課堂通報補上管理員備註對通報者可見,詳見 `docs/PROGRESS.md`/`CLAUDE.md` 第 4.7 節)。使用者提問「課堂通報或異常回報,管理員有留言都要在 tutor/tutee 顯示,目前有這個功能嗎?」,查 code 後發現異常回報已有此功能但課堂通報沒有,經確認後補上:`tutoring/views.py::class_detail()` 新增 `own_resolved_alerts`,`templates/tutoring/class_detail.html` 用既有的 `.completion-box` 樣式顯示已紀錄的通報與管理員備註。純 template + view 變更,**無 migration、無相依套件、無靜態資源異動**,387 項測試全數通過。部署前備份:`/var/backups/mpts/20260912-000428`。`git checkout --detach` 再次卡在根目錄下的 `CLAUDE.md`(同前幾次記錄的既有落差),已用 `sudo -n -u mpts git checkout HEAD -- CLAUDE.md` 補救。只執行 `systemctl restart mpts-gunicorn.service`。驗收:`curl -I` 首頁回應 200;用正式站一組真實 active pairing 的 Tutor 帳號,透過 `manage.py shell` 建立一堂上課中的臨時測試課程、送出課堂通報、以 superuser 標記已紀錄並留備註,確認該 Tutor 的課程詳情頁正確顯示「已紀錄」與備註內容,測試課程(含通報)已刪除;`journalctl` 除既有的 gunicorn `Control server error` 與持續中的 `OpenVAS-VT` 弱掃探測(皆回應 404,與本次部署無關)外,同時觀察到真實使用者(含行動裝置)在部署時間點前後正常瀏覽課表、註冊,無異常。臨時 sudo 授權依使用者指示維持開啟。

- **2026-09-11(六十八)**:操作者 Claude Code(依使用者指示執行)。上一版 `14871b6` → 新版 `258e38a`(Admin「解除配對審核」處理紀錄的「結果 / Result」欄位改用 `.result-text`,拿掉 `.status-badge` 背景色塊只留顏色,使用者要求,詳見 `docs/PROGRESS.md`/`CLAUDE.md`)。純 template class 置換,**無 migration、無相依套件、無靜態資源異動**,386 項測試全數通過。部署前備份:`/var/backups/mpts/20260911-235150`。`git checkout --detach` 再次卡在根目錄下的 `CLAUDE.md`(同前幾次記錄的既有落差),已用 `sudo -n -u mpts git checkout HEAD -- CLAUDE.md` 補救。只執行 `systemctl restart mpts-gunicorn.service`。驗收:`curl -I` 首頁回應 200;用正式站真實 superuser 對 `/dashboard/` 發請求,直接在回應 HTML 裡確認「解除配對審核」處理紀錄的結果欄位已改為 `<span class="result-text status-approved">`,不再是 `.status-badge`;`journalctl` 除既有的 gunicorn `Control server error` 外,另觀察到一批來自 `OpenVAS-VT` UA 的路徑穿越掃描請求(`/file://...etc/passwd` 等),全部正確回應 404,與本次部署無關,判斷是資訊中心排程弱點掃描,非入侵事件。臨時 sudo 授權依使用者指示維持開啟。

- **2026-09-11(六十七)**:操作者 Claude Code(依使用者指示執行)。上一版 `2a46475` → 新版 `14871b6`(異常回報課程欄位完全移除,新增「系統問題」分類,詳見 `docs/PROGRESS.md`/`CLAUDE.md` 第 4.7 節)。使用者指出「課程整個拿掉,因為每堂課有自己的通報,跟當堂課有關就用課程通報,其餘的用異常通報就好」,以及「分類多加一個,系統問題」。**含 1 個 migration**:`tutoring.0036_remove_incidentreport_session_and_more`(`RemoveField` 移除 `session` + `AlterField` 加入 `SYSTEM_ISSUE` 分類,`migrate --plan` 確認)。**部署前資料影響評估並取得使用者明確同意**:部署前用 `manage.py shell` 確認正式站當時有 137 筆真實異常回報、**全部**都還連著課程,已明確告知使用者這個 migration 會永久移除這 137 筆回報與課程的關聯(回報內容/分類/備註本身不受影響),使用者確認「繼續部署(建議先確認可接受)」後才執行。無相依套件變更、無靜態資源變更。386 項測試全數通過(比前一輪部署少 4 項,是移除了「課程選擇/session choices」相關的過時測試,非覆蓋率下降)。部署前備份:`/var/backups/mpts/20260911-234112`(`db.dump` 464K、`media.tar.gz` 3.6M,涵蓋 migration 前的完整 137 筆課程關聯,如需還原可用)。`git checkout --detach` 再次卡在根目錄下的 `CLAUDE.md`(同前幾次記錄的既有落差),已用 `sudo -n -u mpts git checkout HEAD -- CLAUDE.md` 補救。`migrate` 套用後 `systemctl restart mpts-gunicorn.service`;本次 Nginx 設定無變更,未執行 `reload nginx`。驗收:`curl -I` 首頁回應 200;用正式站真實 Tutor 帳號透過 Django test client 驗證表單已無課程欄位、含「系統問題」分類選項,成功送出並在 Dashboard 正確顯示,測試資料已刪除;用正式站 superuser 驗證 Admin dashboard 與行政檔案頁(`admin_user_profile`)皆正常渲染,無 500,「異常回報紀錄」區塊不再連結已不存在的 `report.session_id`;`journalctl` 僅有既有的 gunicorn `Control server error` 無關訊息。臨時 sudo 授權依使用者指示維持開啟。

- **2026-09-11(六十六)**:操作者 Claude Code(依使用者指示執行)。上一版 `27e0ec9` → 新版 `2a46475`(異常回報改為不強制綁定課程,使用者要求:「tutor/tutee 的異常回報不用放課程」、「完全自由填寫,不需要指定對象」,詳見 `docs/PROGRESS.md`/`CLAUDE.md` 第 4.7 節)。`IncidentReport.session` 改為選填,`submit_incident_report()`/表單/畫面同步調整。**含 1 個 migration**:`tutoring.0035_alter_incidentreport_session`(單純 `AlterField` 改為 nullable,無資料遷移,`migrate --plan` 確認)。無相依套件變更、無靜態資源變更(只有 template/Python 變更,不需 `collectstatic`)。390 項測試全數通過。部署前備份:`/var/backups/mpts/20260911-232326`(`db.dump` 456K、`media.tar.gz` 3.6M)。`git checkout --detach` 再次卡在根目錄下的 `CLAUDE.md`(同前幾次記錄的既有落差),已用 `sudo -n -u mpts git checkout HEAD -- CLAUDE.md` 補救。`migrate` 套用後 `systemctl restart mpts-gunicorn.service`;本次 Nginx 設定無變更,未執行 `reload nginx`。驗收:`curl -I` 首頁回應 200;用正式站真實 Tutor 帳號透過 `manage.py shell` 內的 Django test client(`secure=True` 走 HTTPS scheme,避開 `SECURE_SSL_REDIRECT`)驗證 Dashboard 表單正確顯示「不指定特定課程」空值選項、成功送出一筆不選課程的異常回報、Dashboard 正確顯示「未指定課程」的 fallback 文字,測試資料已立即刪除;`journalctl` 僅有既有的 gunicorn `Control server error` 無關訊息,期間並有真實使用者正常完成註冊流程(`/register/` → `/register/confirm/` → `/register/tutee/`)無異常。臨時 sudo 授權依使用者指示維持開啟。

- **2026-09-11(資料異動,非程式碼部署)**:操作者 Claude Code(依使用者指示執行)。使用者於上一筆紀錄後改變主意,決定只保留單一 superuser 帳號(原本的 `admin`),不需要另外的 `superadmin`。部署前備份:`/var/backups/mpts/20260911-162740`。透過 `manage.py shell` 執行:①刪除 `superadmin` 帳號(建立後完全沒有任何活動,`User.delete()` 回報只清掉這一筆 `accounts.User`,無任何關聯資料需要處理);②`admin` 帳號密碼改為使用者指定的新密碼(含大小寫字母、數字、特殊符號,符合 `BilingualPasswordComplexityValidator` 複雜度規則,與上一筆的 `superadmin` 密碼不同,這次不需要繞過驗證)。腳本上傳後執行完立即從本機與 VM 兩端刪除,密碼未寫入任何會保留的檔案。驗收:確認 `superadmin` 已不存在、`admin` 以新密碼可通過 `authenticate()`、正式站目前僅有 1 個 `is_superuser=True` 帳號。臨時 sudo 授權依使用者指示維持開啟。

- **2026-09-11(資料異動,非程式碼部署)**:操作者 Claude Code(依使用者指示執行)。使用者要求建立一個 `superadmin` 帳號,搭配第六十五次部署新增的帳號選單「Django 後台」連結(僅 `is_superuser=True` 可見)使用。部署前備份:`/var/backups/mpts/20260911-153432`。透過 `manage.py shell` 直接以 ORM 建立(非公開註冊、非表單):`username="superadmin"`、`role=ADMIN`、`is_superuser=True`、`is_staff=True`(後者是實際能進入 Django Admin 登入頁的必要條件,不是只有 `is_superuser` 就夠)、密碼為使用者指定的值,直接以 ORM 建立繞過表單驗證(該密碼全小寫、無大寫字母,不符合 `BilingualPasswordComplexityValidator` 的複雜度規則,屬刻意繞過,與先前 `NTNU-OIA-TUTOR`/`NTNU-OIA` 帳號建立時的既有做法一致);建立腳本上傳後執行完立即從本機與 VM 兩端刪除,密碼未寫入任何會保留的檔案。`name_zh`/`name_en` 留空,比照其他管理員帳號的既有慣例,由本人登入 `/profile/` 後自行填寫。驗收:`authenticate()` 確認密碼正確、`force_login()` 後對 `/dashboard/` 發請求確認畫面含「Django 後台」連結、對 `/system-admin/` 發請求確認回應 200。**現況提醒**:建立前確認正式站原本只有最初的 `admin` bootstrap 帳號是 `is_superuser=True`,其餘 9 個一般管理員帳號(`fang.chii`/`fpchang`/`hsinrong`/`jiafeihong`/`jimmy`/`sw.chyu`/`venneasreal`/`xwang`/`zoe`)皆非 superuser,不受影響;建立 `superadmin` 後,正式站上共有 `admin` 與 `superadmin` **兩個** superuser 帳號,原本的 `admin` 帳號並未依此次指示調整,若要讓 `superadmin` 成為唯一能看到「Django 後台」連結的帳號,需要使用者另外明確指示是否要調整 `admin` 帳號的 superuser 狀態。臨時 sudo 授權依使用者指示維持開啟。

- **2026-09-11(六十五)**:操作者 Claude Code(依使用者指示執行)。上一版 `541dd09` → 新版 `f4cb2b3`(使用者回報「後台入口不見了」,帳號選單補上 superuser 專用的 Django 後台連結,詳見 `docs/PROGRESS.md`/`CLAUDE.md`)。**無 migration、無相依套件、無靜態資源異動**,387 項測試全數通過。部署前備份:`/var/backups/mpts/20260911-152745`。`git checkout --detach` 再次卡在根目錄下的 `CLAUDE.md`(既有落差),已用 `sudo -n -u mpts git checkout HEAD -- CLAUDE.md` 補救。只執行 `systemctl restart mpts-gunicorn.service`。驗收:`curl -I` 首頁回應 200;用正式站 superuser 對 `/dashboard/` 發請求確認帳號選單含「Django 後台」連結;`journalctl` 僅有既有的 gunicorn `Control server error` 無關訊息。臨時 sudo 授權依使用者指示維持開啟。

- **2026-09-11(六十四)**:操作者 Claude Code(依使用者指示執行)。上一版 `bd1a460` → 新版 `035100e`(codex review 第二輪回饋三項修正,詳見 `docs/PROGRESS.md`)。**這次分兩部分部署**:
  1. **Nginx 設定**(先於本次 git 部署、獨立完成):把 6 個安全標頭從個別 `location` 提升到 443 `server` 區塊層級,修正「路由前發生的畸形 400 拿不到標頭」這個真實缺口(上一輪 Batch C 誤判為「架構限制」)。部署前備份:`/var/backups/mpts/20260911-151532` 之前已先備份過(見上一筆紀錄的時序);對照範本與正式站現行設定確認除路徑外一致 → `sudo nginx -t` 通過 → 經使用者確認後 `systemctl reload nginx`。**驗收**:用 `printf 'GET / HTTP/1.1\r\n\r\n' | openssl s_client ...`(不帶 `Host` 標頭)在正式站實際重現使用者回報的畸形 400,確認 `Content-Length: 150` 與回報完全吻合,且修正後這 6 個標頭全部正確出現;確認一般 200 回應每個標頭仍只出現 1 次。
  2. **Django 程式碼**:`accounts/views.py::upload_qualification()` 缺檔案的訊息補上英文。**無 migration、無相依套件、無靜態資源變更**,384 項測試全數通過。部署前備份:`/var/backups/mpts/20260911-151532`。`git checkout --detach` 乾淨無衝突(未觸及 `CLAUDE.md`)。部署前額外確認:此時 `/etc/nginx/sites-enabled/mpts.conf` 與 repo 內 `deploy/nginx/mpts.conf.example`(路徑替換 TODO 後)逐字一致,確認先前的 Nginx 手動修正已與這次的 commit 內容同步,沒有落差。只執行 `systemctl restart mpts-gunicorn.service`。**驗收**:`curl -I` 首頁回應 200;用 `TEST-SCAN-TUTOR-NTNU` 對 `/qualification/upload/` 送出缺 `file` 欄位的請求,確認回應內容含完整雙語訊息「此欄位為必填欄位。 / This field is required.」;`journalctl` 僅有既有的 gunicorn `Control server error` 無關訊息。缺失處理報告表(第 3 項修正)的測試數量已同步更新為 384/384,不涉及正式站部署。臨時 sudo 授權依使用者指示維持開啟。

- **2026-09-11(六十三)**:操作者 Claude Code(依使用者指示執行)。上一版 `17534e6` → 新版 `42ef6c9`(使用者回饋:目前在線/累計登入次數這兩張卡片跟上排統計卡太像不好分辨,改成移到下面一張獨立的橫向卡片,新增 `.stat-card-wide` CSS)。**無 migration、無相依套件變更**;**有靜態資源變更**(`collectstatic` 回報「1 static file copied ... 157 unmodified」,`app.css` cache-busting 版本號更新為 `?v=20260911-stat-card-wide`)。384 項測試全數通過。部署前備份:`/var/backups/mpts/20260911-142509`。`git checkout --detach` 這次乾淨無衝突(未觸及 `CLAUDE.md`)。`collectstatic` 後 `systemctl restart mpts-gunicorn.service`。驗收:`curl -I` 首頁回應 200;直接 curl 新版 `app.css` 確認 `.stat-card-wide` 規則已生效;用正式站 superuser 對 `/dashboard/` 發請求確認畫面正確呈現獨立的橫向卡片;`journalctl` 僅有既有的 gunicorn `Control server error` 無關訊息。臨時 sudo 授權依使用者指示維持開啟。
