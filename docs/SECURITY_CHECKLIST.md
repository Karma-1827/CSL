...
| # | 等級 | 控制措施 | 狀態 | CSL 現況與待辦 |
| - | - | - | - | - |
| 56 | 普 | 漏洞修復測試與定期更新 | 🟡 | 2026-07-26 把 `pip-audit` 掛進 CI(`.github/workflows/ci.yml`),每次 push/PR 自動掃 `requirements.txt` 已安裝套件有沒有已知 CVE。**掛上去當天就掃出真的問題**:`Pillow 11.3.0`(42 筆已知漏洞中的一部分,修復版本 12.1.1–12.3.0 不等)、`pypdf 6.10.0`(多筆 CVE/PYSEC,修復版本 6.10.1–6.14.2 不等)都需要升級。CI 步驟目前設 `continue-on-error: true`(不擋 build),因為升級這兩個套件會影響資格文件/課堂紀錄附件的圖片驗證與證明 PDF 合併邏輯,需要另外排時間升版 + 跑一次完整回歸測試(尤其 PDF 那批),不是掛上 CI 當下能一次做完的事。**升級前先看這兩個套件的 changelog 有沒有破壞性 API 變更**,升完要重新跑 `docs/PROGRESS.md`/`CLAUDE.md` 提到的 PDF 人工比對流程。 |
| 57 | 中 | 定期確認漏洞修復狀態 | 🟡 | 掃描機制(見上)已建立且會持續在每次 CI 執行時重新確認;真正做到「定期確認並修復」還要等上述 Pillow/pypdf 升級完成,而且長期應該有人固定看 CI 掃描結果,不是掛了就不用管。 |
...
2. **第 56、57 項:升級 Pillow 與 pypdf 修復已知 CVE** —— `pip-audit` 掃出 `Pillow 11.3.0`、`pypdf 6.10.0` 都有多筆已修復的已知漏洞。這兩個套件分別用在資格文件/課堂紀錄附件的圖片驗證,以及證明 PDF 的疊字合併,升級前要看清楚 changelog 有沒有破壞性 API 變更,升完必須重跑一次完整測試,PDF 相關的還要照 `CLAUDE.md`「PDF 改動」慣例重新產圖人工比對,不是改個版本號就結束。
