...
- tests:`accounts` 47 個、`tutoring` 85 個,共 132 個,**全數通過**(2026-07-26 盤點時實際執行確認)。
...
- **升級 `Pillow`(目前 11.3.0)與 `pypdf`(目前 6.10.0)修復已知 CVE**:`pip-audit`(2026-07-26 掛進 CI)掃出兩者都有多筆已修復的已知漏洞。這兩個套件分別用在資格文件/課堂紀錄附件的圖片格式驗證,以及證明 PDF 疊字合併,升級前要確認 changelog 有沒有破壞性 API 變更,升完要跑完整測試,PDF 相關的還要照 PDF 改動慣例重新產圖人工比對。CI 的 `pip-audit` step 目前設非阻斷,升完級、確認乾淨後應改回阻斷。
